...
percent_var <- lapply(dirs, function(dir){
    readRDS(paste0(dir, '/percent_var.rds'))
})
...
percent_var_grouped <- lapply(dirs, function(dir){
...
AddMatrixByColAndRowNames <- function(A,B){
...
calculateVariableCorrelationMeta <- function(genes_of_interest, dat_variables){
    variable_correlation_abs_matrices <- sapply(genes_of_interest, function(gene_name){
        dat = dat_variables[[gene_name]]
        if(!is.null(dat)){ 
            dat <- dat %>% subset(!is.na(CN))
            if (nrow(dat) > 10) { #genes with at least 10 samples with valid RNA and CN values
                variable_correlation_matrices <- get_variable_correlations(dat)
                variable_correlation_abs_matrix <- variable_correlation_matrices$estimate %>% abs()
                return(variable_correlation_abs_matrix)                
            } else {
                return(NULL) }
        } else {
            return(NULL)
    ...
    variable_correlation_abs_matrices <- 
        variable_correlation_abs_matrices[!sapply(variable_correlation_abs_matrices , is.null)]

    variable_correlation_abs_matrices <- lapply(variable_correlation_abs_matrices, function(matrix){
        peaknames <- colnames(matrix)[grepl('MergePeakID|loop', colnames(matrix))]
        colnames(matrix)[grepl('MergePeakID|loop', colnames(matrix))] <- paste0('PC', seq_along(peaknames))
        rownames(matrix)[grepl('MergePeakID|loop', rownames(matrix))] <- paste0('PC', seq_along(peaknames))
        return(matrix)    
    })

    variable_correlation_abs_matrices_meta <- 
        Reduce(AddMatrixByColAndRowNames, variable_correlation_abs_matrices) / 
        length(variable_correlation_abs_matrices)
    return(variable_correlation_abs_matrices_meta)
}

# meta-analysis for correlation between variables - oncogenes
modeling_runs <- names(dat_PC)
variable_correlation_abs_matrices_meta <- sapply(modeling_runs, function(modeling_run){
    genes_of_interest <- oncogene_names
    calculateVariableCorrelationMeta(genes_of_interest, dat_PC[[modeling_run]])
}, USE.NAMES = TRUE, simplify = FALSE)
variable_correlation_abs_matrices_meta

# meta-analysis for correlation between variables - all genes
modeling_runs <- c('not_cn_regressed_WGS_ploidy_corrected', 'cn_regressed_WGS_ploidy_corrected')
variable_correlation_abs_matrices_all_genes_meta <- sapply(modeling_runs, function(modeling_run){
    genes_of_interest <- modeled_genes[[modeling_run]] %>% unlist
    calculateVariableCorrelationMeta(genes_of_interest, dat_PC[[modeling_run]])
}, USE.NAMES = TRUE, simplify = FALSE)
variable_correlation_abs_matrices_all_genes_meta

mycolors1 <- getColorRampGradient(dark_color = 'firebrick', mid_color = 'white', color_dark_portion = 5/6)

#plot oncogenes
p1 <- Heatmap(variable_correlation_abs_matrices_meta[['not_cn_regressed_WGS_ploidy_corrected']], 
                 column_title = 'No CN regression',
                 col = mycolors1,
                 cluster_rows = FALSE, cluster_columns = FALSE,
                 width = unit(4, 'cm'), height = unit(4, 'cm'))

p2 <- Heatmap(variable_correlation_abs_matrices_meta[['cn_regressed_WGS_ploidy_corrected']], 
                 column_title = 'CN regression',
                 col = mycolors1,
                 cluster_rows = FALSE, cluster_columns = FALSE,
                 width = unit(4, 'cm'), height = unit(4, 'cm'))

pdf('figures/variable_correlations_meta.pdf')
draw(p1 + p2)
dev.off()

#plot all genes
p1 <- Heatmap(variable_correlation_abs_matrices_all_genes_meta[['not_cn_regressed_WGS_ploidy_corrected']], 
                 column_title = 'No CN regression',
                 col = mycolors1,
                 cluster_rows = FALSE, cluster_columns = FALSE,
                 width = unit(4, 'cm'), height = unit(4, 'cm'))

p2 <- Heatmap(variable_correlation_abs_matrices_all_genes_meta[['cn_regressed_WGS_ploidy_corrected']], 
                 column_title = 'CN regression',
                 col = mycolors1,
                 cluster_rows = FALSE, cluster_columns = FALSE,
                 width = unit(4, 'cm'), height = unit(4, 'cm'))

pdf('figures/variable_correlations_all_genes_meta.pdf')
draw(p1 + p2)
dev.off()
...
percent_var_summary[['cn_regressed_WGS_ploidy_corrected_with_cancer_type']] %>% head
percent_var_summary[['cn_regressed_WGS_ploidy_corrected']] %>% head
...
get_percent_var_Cohort <- function(percent_var_summary){
    percent_var_Cohort <- percent_var_summary %>% subset(category=='Cohort')
    percent_var_Cohort$ranked_category <- percent_var_Cohort$category
    rownames(percent_var_Cohort) <- NULL
    return(percent_var_Cohort)
}

get_percent_var_summary_ranked_with_cancer_type <- function(percent_var_summary, model_list){
    percent_var_ranked_EP <- rank_percent_var_EP(percent_var_summary)
    percent_var_CN <- get_percent_var_CN(percent_var_summary)
    percent_var_Cohort <- get_percent_var_Cohort(percent_var_summary)
    percent_var_p_values <- get_percent_var_p_values(percent_var_summary, model_list)
    percent_var_summary_ranked <- rbind(percent_var_ranked_EP, percent_var_CN, percent_var_Cohort) %>% 
        arrange(gene)
    percent_var_summary_ranked <- merge(percent_var_summary_ranked, percent_var_p_values,
                                        by=c('gene','type'), all.x = TRUE)
    return(percent_var_summary_ranked)
}

modeling_run = 'cn_regressed_WGS_ploidy_corrected_with_cancer_type'
percent_var_summary_ranked_with_cancer_type <- get_percent_var_summary_ranked_with_cancer_type(
    percent_var_summary[[modeling_run]],
    model[[modeling_run]])
percent_var_summary_ranked_with_cancer_type %>% head
...
percent_var_CN_EP <- sapply(names(modeled_genes), function(modeling_run){
    ...
    subset(ranked_category=='CN'|ranked_category=='enhancer_component_top_1') %>%
...
for (modeling_run in c('cn_regressed_WGS_ploidy_corrected',
                       'loopcounts_cn_regressed_WGS_ploidy_corrected',
                       'cn_regressed_WGS_ploidy_corrected_with_cancer_type',
                       'loopcounts_not_cn_regressed_WGS_ploidy_corrected',
...
to_plot <- percent_var_CN_EP_subset[['cn_regressed_WGS_ploidy_corrected_with_cancer_type']] %>%
    subset(gene %in% oncogene_names) %>%
    dplyr::arrange(desc(CN)) %>%
    subset(CN_significance != 'NS') %>%
    head(50)
...
pdf('figures/top_CN_genes_barplot_ploidy_corrected_with_cancer_type.pdf', width=4, height=3)
ggplot(to_plot) + 
geom_bar(aes(x=gene, y=CN, fill = CN_significance), stat="identity", position="identity") +
geom_bar(aes(x=gene, y=-1*E_P, fill = EP_significance), stat="identity", position="identity") +
scale_fill_manual(values = mycolors) +
...
pdf('figures/top_EP_genes_barplot_ploidy_corrected_with_cancer_type.pdf', width=4, height=7)
ggplot(to_plot) + 
geom_bar(aes(x=gene, y=CN, fill = CN_significance), stat="identity", position="identity") +
geom_bar(aes(x=gene, y=-1*E_P, fill = EP_significance), stat="identity", position="identity") +
scale_fill_manual(values = mycolors) +
...
percent_var_CN_EP_subset[['cn_regressed_WGS_ploidy_corrected']] %>% head
percent_var_CN_EP_subset[['cn_regressed_WGS_ploidy_corrected_with_cancer_type']] %>% head

gene_order <- percent_var_CN_EP_subset[['cn_regressed_WGS_ploidy_corrected']]$gene %>% as.character 
gene_order %>% head

idx <- match(gene_order, percent_var_CN_EP_subset[['cn_regressed_WGS_ploidy_corrected_with_cancer_type']]$gene)

to_plot <- data.frame(
    gene = gene_order, 
    CN = percent_var_CN_EP_subset[['cn_regressed_WGS_ploidy_corrected']]$CN,
    E_P = percent_var_CN_EP_subset[['cn_regressed_WGS_ploidy_corrected']]$E_P,
    CN_with_cancer_type = percent_var_CN_EP_subset[['cn_regressed_WGS_ploidy_corrected_with_cancer_type']][idx , 'CN'],
    E_P_with_cancer_type = percent_var_CN_EP_subset[['cn_regressed_WGS_ploidy_corrected_with_cancer_type']][idx , 'E_P'])
to_plot %>% head(10)

pdf('figures/fraction_variance_explained_correlation_CN.pdf', useDingbats = FALSE, width = 4, height = 4)
ggplot(to_plot, aes(x = CN, y = CN_with_cancer_type)) +
geom_abline(slope=1, intercept=0, size = 1.5, color = '#999999') +
geom_point(color = '#222222', size = 1) +
theme_classic() +
stat_cor(method = 'pearson') 
dev.off()

pdf('figures/fraction_variance_explained_correlation_E_P.pdf', useDingbats = FALSE, width = 4, height = 4)
ggplot(to_plot, aes(x = E_P, y = E_P_with_cancer_type)) +
geom_abline(slope=1, intercept=0, size = 1.5, color = '#999999') +
geom_point(color = '#222222', size = 1) +
theme_classic() +
stat_cor(method = 'pearson')
...
#percent variance boxplots
percent_var_boxplots <- sapply(c('cn_regressed_WGS_ploidy_corrected', 
                                 'not_cn_regressed_WGS_ploidy_corrected'), 
...
pdf('figures/RNA_modeling_percent_var_boxplots_ploidy_corrected.pdf', width=6, height=6)
grid.arrange(grobs = percent_var_boxplots, ncol = 1)
dev.off()

pdf('figures/RNA_modeling_percent_var_significant_boxplots_ploidy_corrected.pdf', width=6, height=6)
grid.arrange(grobs = percent_var_significant_boxplots, ncol = 1)
dev.off()

#percent variance boxplots for oncogenes
genes_of_interest <- oncogene_names

percent_var_boxplots_oncogenes <- sapply(c('cn_regressed_WGS_ploidy_corrected', 
                                           'not_cn_regressed_WGS_ploidy_corrected'), 
                               function(modeling_run){
    dat_percent_var_categories <- get_dat_percent_var_categories(percent_var_summary_ranked[[modeling_run]]) %>%
                                   subset(gene %in% genes_of_interest)
    plot <- plot_percent_var_boxplot(dat_percent_var_categories) + ggtitle(modeling_run)
    return(plot)
}, simplify=FALSE, USE.NAMES=TRUE)

percent_var_significant_boxplots_oncogenes <- sapply(c('cn_regressed_WGS_ploidy_corrected', 
                                                       'not_cn_regressed_WGS_ploidy_corrected'), 
                                           function(modeling_run){
    dat_percent_var_categories <- get_dat_percent_var_categories(percent_var_summary_ranked[[modeling_run]],
                                                                coeff_p_value_cutoff=0.05) %>% 
...
pdf('figures/RNA_modeling_percent_var_boxplots_oncogenes_ploidy_corrected.pdf', width=6, height=6)
grid.arrange(grobs = percent_var_boxplots_oncogenes, ncol = 1)
dev.off()

pdf('figures/RNA_modeling_percent_var_significant_boxplots_oncogenes_ploidy_corrected.pdf', width=6, height=6)
grid.arrange(grobs = percent_var_significant_boxplots_oncogenes, ncol = 1)
dev.off()

categories = c('CN', paste0('enhancer_component_top_', 1:5), 'Cohort', 'total')

to_plot <- percent_var_summary_ranked_with_cancer_type %>%
...
to_plot2 <- rbind(to_plot, to_plot_total) %>% 
    arrange(gene) %>% 
    mutate(ranked_category = factor(ranked_category, levels = categories))
head(to_plot2)

pdf('figures/RNA_modeling_percent_var_boxplots_ploidy_corrected_with_cancer_type.pdf', width=6, height=3)
plot_percent_var_boxplot(to_plot2)
dev.off()
...
dir.create('figures/scatter_EP_CN_RNA_ploidy_corrected')
